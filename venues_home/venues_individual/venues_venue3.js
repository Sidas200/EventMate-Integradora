document.addEventListener('DOMContentLoaded', async () => {
    ...
    const venue = {
        ...
        tagline: "Donde tus sueños se hacen realidad",
    ...
    // Function to fetch and display comments for venue3
    async function fetchCommentsForVenue3() {
        try {
            const response = await fetch('https://eventmate-integradora.onrender.com/comentarios_venue3');
            if (response.ok) {
                const comments = await response.json();
                displayComments(comments);
            } else {
                console.error('Error fetching comments for venue3:', response.statusText);
            }
        } catch (error) {
            console.error('Error fetching comments for venue3:', error);
        }
    }
    ...
    // Function to handle adding a comment for venue3
    async function addCommentForVenue3(event) {
        event.preventDefault();
        const newComment = commentText.value.trim();
        if (newComment) {
            try {
                const response = await fetch('https://eventmate-integradora.onrender.com/comentario_venue3', {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    console.log('Comment added successfully for venue3');
                    fetchCommentsForVenue3(); // Refresh comments after adding
                    commentText.value = ''; // Clear the input field
                } else {
                    console.error('Error adding comment for venue3:', response.statusText);
                }
            } catch (error) {
                console.error('Error adding comment for venue3:', error);
            }
        }
    }
    ...
    commentForm.addEventListener('submit', addCommentForVenue3);
    ...
    fetchCommentsForVenue3(); // Fetch and display existing comments for venue3
});
